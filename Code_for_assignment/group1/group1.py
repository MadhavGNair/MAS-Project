...
class Group1(SAONegotiator):
    ...
    def compute_concession(self, state: SAOState):
        ...
        if self.phase == 1:
            ...
            # beta value in phase 2 when the opponent has the final bid
            beta = 3
        elif self.phase == 2 and not self.opponent_ends:
            # beta value in phase 2 when we have the final bid
            beta = 4.5
        elif self.phase == 3 and self.opponent_ends:
            # beta value in phase 3 when the opponent has the final bid
            beta = 0.5
        else:
            # beta value in phase 3 when we have the final bid
            beta = 1
        
        # Concession curve formula (explained in the report)
